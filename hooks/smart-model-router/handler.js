...
const MODEL_ROUTER_PATH = "/Users/liulaoshi/.openclaw/workspace/model_router.py";
...
if (typeof window !== 'undefined') {
  window.SmartRouter = {
    ...
    getStatus: async () => {
      try {
        const response = await fetch('http://localhost:8000/api/smart-router/status');
...
// 获取 Shell 包装脚本
function getShellWrapperScript() {
  return `#!/bin/bash
# 智能模型调用包装器
